...
u_final_expm = scipy.sparse.linalg.expm_multiply(1 / h**2 * D_0_square, u[0])
print("time it took with expm_multiply: ", time.time() - start_time)
...
for t in time_grid:
    ...
    u_prev = u[-1]
    u_next = u_prev + delta / h**2 * (D_0_square @ u_prev)
    u.append(u_next)
...
u_final_ee = u[-1]
u_final_exact = np.exp(-1) * np.sin(space_grid)

err_expm = np.linalg.norm(u_final_expm - u_final_exact, ord=2) * np.sqrt(
    np.pi / (N + 1)
)

err_ee = np.linalg.norm(u_final_ee - u_final_exact, ord=2) * np.sqrt(np.pi / (N + 1))

print("error of expm solution at the last time: ", err_expm)
