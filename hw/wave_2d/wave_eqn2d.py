...
class WaveEqn2D:
    def __init__(self, nx=500, ny=500, c=0.2, h=1, dt=1, use_mur_abc=True):
    ...
    def update(self):
        ...
        u[0, 1 : ny - 1, 1 : nx - 1] = self.alpha2 * (
            ...
            + u[1, 1 : ny - 1, 2:nx]
            - 4 * u[1, 1 : ny - 1, 1 : nx - 1]
        ) + (2 * u[1, 1 : ny - 1, 1 : nx - 1] - u[2, 1 : ny - 1, 1 : nx - 1])

        if self.use_mur_abc:
            # Mur absorbing boundary conditions.
            kappa = (1 - self.alpha) / (1 + self.alpha)
            u[0, 0, 1 : nx - 1] = u[1, 1, 1 : nx - 1] - kappa * (
                u[0, 1, 1 : nx - 1] - u[1, 0, 1 : nx - 1]
            )
            u[0, ny - 1, 1 : nx - 1] = u[1, ny - 2, 1 : nx - 1] + kappa * (
                u[1, ny - 1, 1 : nx - 1] - u[0, ny - 2, 1 : nx - 1]
            )
            u[0, 1 : ny - 1, 0] = u[1, 1 : ny - 1, 1] - kappa * (
                u[0, 1 : ny - 1, 1] - u[1, 1 : ny - 1, 0]
            )
            u[0, 1 : ny - 1, nx - 1] = u[1, 1 : ny - 1, nx - 2] + kappa * (
                u[1, 1 : ny - 1, nx - 1] - u[0, 1 : ny - 1, nx - 2]
            )
