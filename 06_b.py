...
for i in range(N_time_steps):
    ...
    # diffusion
    g_d = np.zeros_like(u)
    g_d[0] = rx * np.exp(-0.1 * t_n) * np.sin(-2 * t_n_half)
    g_d[-1] = rx * np.exp(-0.1 * t_n) * np.sin(3 - 2 * t_n_half)

    u_half = u - mu * D_upw @ u + g_a
    u = u_half + rx * D_c_2 @ u_half + g_d
